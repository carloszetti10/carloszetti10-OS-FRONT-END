...
import { usePodeGerenciarPermissoes, usePodeConfigurarBitrix } from "@/hooks/usePermissoes"; 
...
export function Sidebar({ recolhida, aoAlternar }: SidebarProps) { 
  ...
  const podeConfigurarBitrix = usePodeConfigurarBitrix(); 
  ...
  const SUBMENU_CONFIGURACAO = [ 
    ...
    ...(podeConfigurarBitrix ? [{ to: "/configuracoes/bitrix", label: "Bitrix" }] : []), 
    ...(podeGerenciarPermissoes ? [{ to: "/permissoes", label: "Permissões" }] : []), 
  ]; 
